...
for fn in ["train", "dev", "test"]:
    data = jsonlines.Reader(open('C:/Users/mrper/Desktop/24[7]ai/GettingToKnowYou-master/data/dialogue_nli_extra/dialogue_nli_EXTRA_uu_{}.jsonl'.format(fn), 'r')).read()
    pbar = tqdm(data)
    for d in pbar:
        try:
            if remove_punctuation(d["sentence1"]) not in s2t_map.keys():
...
for fn in ["valid", "test", "train"]:
    ...
    fw = open("C:/Users/mrper/Desktop/24[7]ai/GettingToKnowYou-master/data/ConvAI2/{}_both_original_dialogueNLI.txt".format(fn), "w")
    lines = fr.readlines()
